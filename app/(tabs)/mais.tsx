...
function MenuRow({ icon: Icon, iconColor, iconBg, label, desc, onPress, isLast }: {
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => ({ backgroundColor: pressed ? "#F8FAFC" : "transparent" })}>
      {/* Inner View handles flexDirection row */}
      <View style={{
        flexDirection: "row",
        alignItems: "center",
        paddingVertical: 14,
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.background }} edges={["top"]}>
      ...
      <ScrollView style={{ flex: 1, backgroundColor: Colors.page }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginHorizontal: 20, marginTop: 12, borderRadius: 16, overflow: "hidden", elevation: 4, shadowColor: Colors.primary.DEFAULT, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 12 }}>
          <LinearGradient
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <View style={{ width: 50, height: 50, borderRadius: 16, backgroundColor: "rgba(255,255,255,0.25)", alignItems: "center", justifyContent: "center" }}>
                <Text style={{ fontSize: 18, fontFamily: "Inter_700Bold", color: "#FFFFFF" }}>
                  {(profile?.nome || "U").split(" ").map((w: string) => w[0]).join("").slice(0, 2).toUpperCase()}
                </Text>
              </View>
              <View style={{ flex: 1, marginLeft: 14 }}>
        ...
        <View style={{ marginHorizontal: 20, backgroundColor: Colors.card, borderRadius: 16, borderWidth: 1, borderColor: Colors.border.DEFAULT, elevation: 3, shadowColor: "#0F172A", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, overflow: "hidden" }}>
        ...
        <View style={{ marginHorizontal: 20, marginBottom: 40, backgroundColor: Colors.card, borderRadius: 16, borderWidth: 1, borderColor: Colors.border.DEFAULT, elevation: 3, shadowColor: "#0F172A", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, overflow: "hidden" }}>
          <MenuRow icon={User} iconColor={Colors.primary.DEFAULT} iconBg={Colors.primary.bg} label="Meu Perfil" onPress={() => {}} />
          <MenuRow icon={Settings} iconColor={Colors.text.secondary} iconBg={Colors.muted} label="Configurações" onPress={() => {}} isLast />

          {/* Logout - also uses inner View for row */}
          <Pressable onPress={handleLogout} style={({ pressed }) => ({ backgroundColor: pressed ? Colors.destructive.bg : "transparent" })}>
            <View style={{
              flexDirection: "row",
              alignItems: "center",
              paddingVertical: 14,
