...
export default function MunicipesListScreen() {
  ...
  const { data: municipes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("municipes")
        .select("id, nome, telefone, email, bairro, categoria_id, municipe_categorias(nome, cor, icone), demandas(id)")
        .order("nome")
        .limit(500);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.background }} edges={["top"]}>
      ...
      <SearchBar placeholder="Buscar por nome, telefone..." value={searchTerm} onChangeText={setSearchTerm} onFilterPress={() => {}} />
      <FlatList
        data={filtered}
        ...
        ListEmptyComponent={<EmptyState icon="👥" title="Nenhum munícipe encontrado" subtitle={searchTerm ? "Tente outro termo" : "Cadastre o primeiro munícipe"} />}
